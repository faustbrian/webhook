...
beforeEach(function (): void {
    ...
    // Register webhook routes manually with configName parameter
    Route::post('/webhooks/{configName?}', WebhookController::class)
        ->defaults('configName', 'default')
        ->name('webhook.default');

    Route::post('/webhooks/custom/{configName?}', WebhookController::class)
...
describe('WebhookController', function (): void {
    describe('Happy Path', function (): void {
        ...
        test('delegates to WebhookProcessor with default config name', function (): void {
            ...
            Event::assertDispatched(WebhookReceivedEvent::class, function ($event): bool {
                return $event->configName === 'default';
            });
        });
        ...
        test('delegates to WebhookProcessor with custom config name from route', function (): void {
            ...
            Event::assertDispatched(WebhookReceivedEvent::class, function ($event): bool {
                return $event->configName === 'custom';
            });
        });
        ...
        test('returns processor response directly without modification', function (): void {
            ...
            $mockProfile->shouldReceive('shouldProcess')->andReturn(true);
            $mockResponse->shouldReceive('response')->andReturn(
                new Response('Custom processor response', 202),
            );
        ...
        test('queues processing job via processor', function (): void {
            ...
            // Assert
            Queue::assertPushed(ProcessWebhookJob::class, function ($job): bool {
                return $job->webhookCall->webhook_id === 'webhook-job';
            });
        });
    });
    ...
    describe('Integration with Laravel Features', function (): void {
        test('route is registered via webhooks macro', function (): void {
            // Arrange - Route already registered in beforeEach

            // Act
            $registeredRoutes = Route::getRoutes();
            $webhookRoute = $registeredRoutes->getByName('webhook.default');

            // Assert
            expect($webhookRoute)->not->toBeNull();
            expect($webhookRoute->uri())->toBe('webhooks');
            expect($webhookRoute->methods())->toContain('POST');
        });
        ...
        test('route excludes CSRF verification middleware', function (): void {
            ...
            $response = $this->post('/webhooks', ['event' => 'csrf.test'], [
        ...
        test('custom route name is set correctly', function (): void {
            // Arrange - Custom route already registered in beforeEach

            // Act
            $registeredRoutes = Route::getRoutes();
            $customRoute = $registeredRoutes->getByName('webhook.custom');

            // Assert
            expect($customRoute)->not->toBeNull();
            expect($customRoute->uri())->toBe('webhooks/custom');
            expect($customRoute->methods())->toContain('POST');
        });
