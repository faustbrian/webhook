...
#[Small()]
final class WebhookCallTest extends \Tests\TestCase
{
    ...
    public static function provideThrows_exception_for_malformed_urlsCases(): iterable
    {
        yield from [
            'empty string' => [''],
            'missing protocol' => ['example.com/webhook'],
            'invalid characters' => ['http://example .com'],
            'just a path' => ['/webhook'],
    ...
    public function uses_custom_signer_over_signature_version(): void
    {
        ...
        // Assert
        Queue::assertPushed(CallWebhookJob::class, function ($job) use ($customSigner) {
            $reflection = new ReflectionClass($job);
            $property = $reflection->getProperty('signer');
            $property->setAccessible(true);
    ...
    public function uses_custom_webhook_id_over_generated_id(): void
    {
        ...
        // Assert
        Queue::assertPushed(CallWebhookJob::class, function ($job) use ($customId) {
            $reflection = new ReflectionClass($job);
    ...
    public function handles_various_case_http_verbs(string $verb, string $expected): void
    {
        ...
        // Assert
        Queue::assertPushed(CallWebhookJob::class, function ($job) use ($expected) {
            $reflection = new ReflectionClass($job);
            $property = $reflection->getProperty('httpVerb');
            $property->setAccessible(true);
    ...
    public function handles_url_with_special_characters(): void
    {
        ...
        // Assert
        Queue::assertPushed(CallWebhookJob::class, function ($job) use ($url) {
            $reflection = new ReflectionClass($job);
    ...
    public function handles_deeply_nested_payload(): void
    {
        ...
        // Assert
        Queue::assertPushed(CallWebhookJob::class, function ($job) use ($payload) {
            $reflection = new ReflectionClass($job);
            $property = $reflection->getProperty('payload');
            $property->setAccessible(true);
