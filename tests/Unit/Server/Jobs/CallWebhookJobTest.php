...
describe('CallWebhookJob', function (): void {
    ...
    describe('Event Dispatching Patterns', function (): void {
        test('dispatches DispatchingWebhookCallEvent on successful call', function (): void {
            // Arrange
            $job = createJob(url: 'https://httpbin.org/status/200');
        ...
        test('dispatches WebhookCallSucceededEvent on 2xx response', function (): void {
            // Arrange
            $job = createJob(url: 'https://httpbin.org/status/200');
        ...
        test('dispatches WebhookCallFailedEvent on error', function (): void {
            // Arrange
            $job = createJob(
        ...
        test('dispatches FinalWebhookCallFailedEvent after max retries', function (): void {
            // Arrange
            $job = createJob(
                url: 'https://httpbin.org/status/500',
                tries: 1,
            );
            setJobAttempts($job, 1); // Simulate final attempt
            ...
            // Assert
            Event::assertDispatched(FinalWebhookCallFailedEvent::class);
        });
    });

    describe('HTTP Response Handling', function (): void {
        test('handles 2xx status codes as success', function (): void {
            // Arrange
            $job = createJob(url: 'https://httpbin.org/status/201');
        ...
        test('handles 3xx status as failure', function (): void {
            // Arrange - Use 304 Not Modified which won't redirect
            $job = createJob(url: 'https://httpbin.org/status/304', tries: 1, throwExceptionOnFailure: true);
            setJobAttempts($job, 1);

            // Act & Assert
            expect(fn () => $job->handle())->toThrow(MaxRetriesExceededException::class);
            Event::assertDispatched(FinalWebhookCallFailedEvent::class);
        });

        test('handles 4xx status as failure', function (): void {
            // Arrange
            $job = createJob(url: 'https://httpbin.org/status/404', tries: 1, throwExceptionOnFailure: true);
            setJobAttempts($job, 1);
        ...
        test('handles 5xx status as failure', function (): void {
            // Arrange
            $job = createJob(url: 'https://httpbin.org/status/503', tries: 1, throwExceptionOnFailure: true);
            setJobAttempts($job, 1);
        ...
        test('re-throws exception when retries remain', function (): void {
            // Arrange
            $job = createJob(url: 'https://httpbin.org/status/500', tries: 3);
            setJobAttempts($job, 1); // First attempt out of 3

            // Act & Assert
            expect(fn () => $job->handle())->toThrow(WebhookCallException::class);
            Event::assertDispatched(WebhookCallFailedEvent::class);
            Event::assertNotDispatched(FinalWebhookCallFailedEvent::class);
        });

        test('throws MaxRetriesExceededException when configured', function (): void {
            // Arrange
            $job = createJob(
                url: 'https://httpbin.org/status/500',
                tries: 1,
...
function setJobAttempts(CallWebhookJob $job, int $attempts): void
{
    ...
    $mockJob = new readonly class($attempts)
    {
