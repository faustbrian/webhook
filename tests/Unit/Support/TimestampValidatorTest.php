...
describe('TimestampValidator', function (): void {
    ...
    describe('validate() - exceptions', function (): void {
        ...
        test('throws ExpiredTimestampException with correct message', function (): void {
            ...
            } catch (ExpiredTimestampException $expiredTimestampException) {
                expect($expiredTimestampException->getMessage())
                    ->toContain(sprintf('Webhook timestamp (%d) is too old', $timestamp))
                    ->toContain('Age: 500s')
        ...
        test('throws FutureTimestampException with correct message', function (): void {
            ...
            try {
                $validator->validate($timestamp);
                expect(true)->toBeFalse('Exception should have been thrown');
            } catch (FutureTimestampException $futureTimestampException) {
                expect($futureTimestampException->getMessage())
                    ->toContain(sprintf('Webhook timestamp (%d) is in the future', $timestamp))
