...
beforeEach(function (): void {
    // Use ULID for primary keys (matching TestCase configuration)
...
describe('WebhookProcessor', function (): void {
    describe('Happy Path', function (): void {
        ...
        test('uses custom config name correctly', function (): void {
            ...
            Event::assertDispatched(WebhookReceivedEvent::class, fn ($event): bool => $event->configName === 'stripe');
        });
    });
    ...
    describe('Integration Tests', function (): void {
        ...
        test('webhook response handler receives created WebhookCall', function (): void {
            ...
            $mockResponse->shouldReceive('response')
                ->once()
                ->with(Mockery::on(fn ($webhookCall): bool => $webhookCall instanceof WebhookCall
                    && $webhookCall->webhook_id === 'webhook-response'
                    && $webhookCall->config_name === 'default'))
                ->andReturn(
                    new Response('Custom Response', \Symfony\Component\HttpFoundation\Response::HTTP_ACCEPTED),
                );
